{
   ...
   TGraph* gr = new TGraph("ZScore-Calculator-Mean-All.tpl","%lg %lg");
   gr->SetTitle("Background dependence of mean (numerical estimate)");
//   gr->SetName("0.125 mrad");
   ...
   gr->GetYaxis()->SetTitle("Mean of #it{Z}^{S}_{N}");
//   TAxis xaxis=gr->GetXaxis();
   gr->GetXaxis()->SetLimits(0.05, 330.0);
   gr->SetMaximum(0.08);
   gr->SetMinimum(-0.01);
...
//   t->DrawLatex(0.5,3.5,"#pm 1 #sigma band (#chi^{2}/#nu = 2.3/4)");
   ...
   t2->SetTextSize(0.04);
   t2->SetTextAlign(12);
   t2->DrawLatex(1.0,0.065,"Evaluated in the Poisson limit (#hat{#sigma}_{b} = 0)");   
