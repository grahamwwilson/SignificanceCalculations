...
// Read values from input   
   if(argc==5){
       ...
       NOBS = atoi(argv[2]);                 // Number of observed events
       MUB = double(atof(argv[3]));          // Expected background
       FRACERROR = double(atof(argv[4]));    // Used to define the smearing of the expected value
...
//   hist->Draw();
   ...
   if(NOBS >= MUB){
       cout << "NOBS >= MUB:  z-score upper (ie. signed pull equivalent): " << TMath::NormQuantile(1.0-pvalueu) << " sigma " << endl;
   }    
   else{
       cout << "NOBS < MUB :  z-score lower (ie. signed pull equivalent): " << -TMath::NormQuantile(1.0-qvaluel) << " sigma " << endl;
   }
// Alternative formulation based on measured probabilities 
   if(pvalueu < qvaluel){
       cout << "pu < ql    :  z-score upper (ie. signed pull equivalent): " << TMath::NormQuantile(1.0-pvalueu) << " sigma " << endl;   
   }
   else if(qvaluel < pvalueu){
       cout << "ql < pu    :  z-score lower (ie. signed pull equivalent): " << -TMath::NormQuantile(1.0-qvaluel) << " sigma " << endl;   
   }
   else{
